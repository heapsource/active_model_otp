...
class OtpTest < MiniTest::Unit::TestCase
  def setup
    ...
    @guest = Guest.new
    @guest.email = 'roberto@heapsource.com'
    @guest.run_callbacks :create
  end
  ...
  def test_authenticate_with_otp_when_interval_is_set
    code = @guest.otp_code(Time.now - 30)
    assert @guest.authenticate_otp(code)
  end
  ...
  def test_provisioning_uri_with_provided_account
    ...
    assert_match %r{otpauth://totp/roberto\?secret=\w{16}}, @guest.provisioning_uri("roberto")
  end
  ...
  def test_provisioning_uri_with_email_field
    ...
    assert_match %r{otpauth://totp/roberto@heapsource\.com\?secret=\w{16}}, @guest.provisioning_uri
  end
  ...
  def test_provisioning_uri_with_options
    ...
    assert_match %r{otpauth://totp/roberto@heapsource\.com\?issuer=Example&secret=\w{16}}, @guest.provisioning_uri(nil,issuer: "Example")
    ...
    assert_match %r{otpauth://totp/roberto\?issuer=Example&secret=\w{16}}, @guest.provisioning_uri("roberto", issuer: "Example")
  end
